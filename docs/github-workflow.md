...
Ordinary changes target `dev` and use the default pull-request template. The description records the issue, summary, verification evidence, operational impact, and agent assistance. A human reviews and squash-merges each feature branch so the resulting `dev` commit follows Conventional Commits.
